...
set "CONDA_ENV_DIR="
set "EXE_FILE=%APP_DIR%AutoLabeler.exe"
...
rem The exe will use this Python only for training/auto-label/validation subprocesses.
if defined CONDA_ENV_DIR (
    if exist "%CONDA_ENV_DIR%\python.exe" (
        echo [AutoLabeler] External Python:
        echo %CONDA_ENV_DIR%\python.exe
        set "AUTOLABELER_PYTHON_EXE=%CONDA_ENV_DIR%\python.exe"
        set "AUTOLABELER_CONDA_ENV_DIR=%CONDA_ENV_DIR%"
    ) else (
    ...
    echo [AutoLabeler] No yolo_stable conda environment found.
    echo [AutoLabeler] Falling back to Python found by the application.
)
echo.
...
if errorlevel 1 (
    echo.
    echo [ERROR] AutoLabeler exited with error code %errorlevel%.
    pause
    exit /b %errorlevel%
)
