...
def build_clean_python_env(base_env: dict[str, str] | None = None) -> dict[str, str]:
    """외부 Python이 현재 exe의 Python 설정을 잘못 물려받지 않도록 환경 변수를 정리합니다."""
    ...
    if conda_env_dir:
        ...
        # conda activate 없이도 외부 Python이 필요한 DLL을 먼저 찾도록 PATH 앞쪽만 보정합니다.
        conda_paths = [
            env_path,
...
def clean_windows_dll_search_path() -> Iterator[None]:
    """PyInstaller DLL 검색 경로가 외부 Python에 상속되지 않도록 잠시 초기화합니다."""
    if os.name != "nt":
        yield
        return
